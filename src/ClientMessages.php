...
class ClientMessages
{
     ...
     * <br>
     *
     * For convenience, you may pass a {@link TelegramObject TelegramObject} to the first argument which contains
     ...
     */
    public function acceptUrlAuth(mixed $messageId, int $buttonId = 0, array $extra = []): TelegramObject
    {
     ...
     */
    public function addChatUser(mixed $chatId, mixed $userId = '', int $forwardLimit = 1): TelegramObject
    {
     ...
     */
    public function checkChatInvite(mixed $hash): TelegramObject
    {
     ...
     * @return bool Bool
     */
    public function clearRecentStickers(TelegramObject|bool $attached = false): bool
    {
     ...
     * @return TelegramObject Updates
     */
    public function createChat(mixed $title, array $users = []): TelegramObject
    {
     ...
     * @return TelegramObject Updates
     */
    public function deleteChatUser(mixed $chatId, mixed $userId): TelegramObject
    {
        if ($chatId instanceof TelegramObject) {
            $payload = $chatId->toArray();
        } else {
     ...
     */
    public function deleteHistory(mixed $peer, int $maxId = 0, array $extra = []): TelegramObject
    {
     ...
     * @return TelegramObject Updates
     */
    public function deleteScheduledMessages(mixed $peer, array $id = []): TelegramObject
    {
        if ($peer instanceof TelegramObject) {
            $payload = $peer->toArray();
        } else {
     ...
     */
    public function editChatAbout(mixed $peer, string $about = ''): TelegramObject
    {
        if ($peer instanceof TelegramObject) {
            $payload = $peer->toArray();
        } else {
     ...
     * @return bool Bool
     */
    public function editChatAdmin(mixed $chatId, mixed $userId, bool $isAdmin): bool
    {
        if ($chatId instanceof TelegramObject) {
            $payload = $chatId->toArray();
        } else {
     ...
     * @return TelegramObject Updates
     */
    public function editChatDefaultBannedRights(mixed $peer, array $bannedRights): TelegramObject
    {
        if ($peer instanceof TelegramObject) {
            $payload = $peer->toArray();
        } else {
     ...
     * @return TelegramObject Updates
     */
    public function sendMessage(mixed $peer, string $message = '', array $extra = []): TelegramObject
    {
        if ($peer instanceof TelegramObject) {
            $payload = $peer->toArray();
        } else {
     ...
     * @return TelegramObject
     */
    public function getHistory(TelegramObject|array $params): TelegramObject
    {
